...
class GLVBO(GLReferenceCounter):
    ...
    def __init__(self, renderType, vertexArray,
                 normalArray=None, indicesArray=None, colorArray=None):
        ...
        if not bool(glGenBuffers):  # Fallback if buffers are not supported.
            ...
            if self._hasNormals:  # TODO: Add size check to see if arrays have same size.
                ...
                glBufferData(GL_ARRAY_BUFFER, numpy.concatenate(
                    (vertexArray, normalArray), 1), GL_STATIC_DRAW)
            else:
                if self._hasColor:
                    glPointSize(1)
                    ...
                    glBufferData(
                        GL_ARRAY_BUFFER, numpy.array(colorArray, numpy.uint8), GL_STATIC_DRAW)
                else:
                    self._buffer = glGenBuffers(1)
                    glBindBuffer(GL_ARRAY_BUFFER, self._buffer)
                    glBufferData(GL_ARRAY_BUFFER, vertexArray, GL_STATIC_DRAW)
            ...
            if self._hasIndices:
                ...
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, numpy.array(
                    indicesArray, numpy.uint32), GL_STATIC_DRAW)
    ...
    def render(self):
        ...
        if self._hasIndices:
            ...
            # Warning, batchSize needs to be dividable by 4 (quads), 3 (triangles) and
...
def loadGLTexture(filename):
    ...
    if alphaData is not None:
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img.GetWidth(),
                     img.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, rgbData)
    return tex
